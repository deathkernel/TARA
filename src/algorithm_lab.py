...
selection; it does not assume that generated code is novel or correct.
"""
...
from .algorithm_discovery import AlgorithmCandidate, DiscoveryResult, discover_algorithms
from .polyglot.archive import CandidateArchive
from .polyglot.benchmark import BenchmarkResult, PolyglotBenchmark, rank_benchmarks
...
class CandidateDiversity:
    """Cheap token/character similarity used to avoid selecting clones."""
...
class MultiObjectiveSelector:
    """Rank verified candidates using normalized multi-objective utility."""
    ...
    def select(self, results: Sequence[BenchmarkResult], novelty: Sequence[NoveltyReport]) -> tuple[BenchmarkResult, ...]:
        ...
        while verified:
            best = max(
                verified,
                key=lambda result: self._utility(
                    result, selected, novelty, minimum, maximum
                ),
            )
    ...
    def _utility(self, result, selected, novelty, minimum, maximum):
        ...
        novel = 1.0
        for report in novelty:
            if report.fingerprint == result.candidate.fingerprint:
                novel = 1.0 if report.new_to_archive else max(0.0, 1.0 - report.nearest_similarity)
                break
...
class AlgorithmDiscoveryLab:
    """Run the full generate -> execute -> verify -> benchmark -> learn loop."""

    def __init__(self, *, benchmark=None, archive=None, novelty=None):
    ...
    def discover(
        ...
        *,
        ...
        novelty = tuple(self.novelty.analyze(problem.name, result.candidate.language, result.candidate.source)
                        for result in benchmarks) if self.novelty else ()
        selected = self.selector.select(benchmarks, novelty)
        scores = tuple(self._score(result, novelty, generated) for result in selected)
    ...
    @staticmethod
    def _score(result, novelty, all_candidates):
        report = next((item for item in novelty if item.fingerprint == result.candidate.fingerprint), None)
        novelty_score = 1.0 if report is None or report.new_to_archive else max(0.0, 1.0 - report.nearest_similarity)
...
class AdaptiveAlgorithmSearch:
    """Compatibility adapter around the existing iterative improvement engine."""
